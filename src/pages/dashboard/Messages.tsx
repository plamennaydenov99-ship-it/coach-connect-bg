...
const Messages = () => {
  ...
  return (
    <div className="max-w-6xl space-y-6">
      <div>
        <h1 className="font-display text-3xl">{t.dashmsg_title}</h1>
        <p className="text-muted-foreground mt-1">{t.dashmsg_sub}</p>
      </div>

      <div className="surface overflow-hidden grid md:grid-cols-[280px_1fr] h-[calc(100vh-220px)] min-h-[500px]">
        <div className="border-r border-border overflow-y-auto">
          {loading ? (
            <p className="p-4 text-sm text-muted-foreground">{t.dashmsg_loading}</p>
          ) : convos.length === 0 ? (
            ...
            >
              <img
                ...
                alt={`Profile photo of ${c.other?.full_name || t.dashmsg_user}`}
                className="h-10 w-10 rounded-full object-cover shrink-0"
              />
              <div className="min-w-0 flex-1">
                <p className="font-medium text-sm truncate">{c.other?.full_name || t.dashmsg_user}</p>
            ...
            <>
              <div className="p-4 border-b border-border flex items-center gap-3">
                <img
                  ...
                  alt={`Profile photo of ${active.other?.full_name || t.dashmsg_user}`}
                  className="h-9 w-9 rounded-full object-cover"
                />
                <p className="font-medium">{active.other?.full_name || t.dashmsg_user}</p>
              </div>
              ...
              <div className="px-4 py-2 border-t border-border flex items-center gap-2 text-[11px] text-muted-foreground bg-background-secondary/50">
                ...
                <span>{t.dashmsg_safety}</span>
              </div>
              <form onSubmit={send} className="p-3 border-t border-border flex gap-2">
                <Input value={draft} onChange={e => setDraft(e.target.value)} placeholder={t.dashmsg_reply_ph} />
