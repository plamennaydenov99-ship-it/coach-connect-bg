...
export function EventsSlider() {
  ...
  useEffect(() => {
    const tm = setInterval(next, 4000);
  ...
  return (
    <section className="container py-16 md:py-20">
      <div className="mb-8 flex items-end justify-between gap-4">
        <div>
          <h2 className="font-display">{t.events_upcoming}</h2>
        ...
        <Link to="/events" className="hidden md:block">
          <Button variant="ghost">
            {t.events_view_all} <ArrowRight className="h-4 w-4 ml-2" />
          </Button>
        </Link>
      </div>

      <div
        aria-label={t.events_featured_aria}
      ...
      >
            ...
            <article key={ev.id} className="relative w-full shrink-0" aria-roledescription="slide">
              <div className="relative aspect-[21/9] w-full overflow-hidden md:aspect-[21/8]">
                ...
                <div className="absolute inset-x-0 bottom-0 p-6 md:p-10">
                  ...
                  <Button className="mt-5">{t.events_view_event}</Button>
                </div>
              </div>
            </article>
          ))}
        ...
        <button
          onClick={prev}
          aria-label={t.events_prev_slide}
        ...
        <button
          onClick={next}
          aria-label={t.events_next_slide}
      ...
      <div className="mt-4 flex justify-center gap-1.5">
        {EVENTS.map((_, i) => (
          <button
            key={i}
            onClick={() => setIndex(i)}
            aria-label={`${t.events_go_to_slide} ${i + 1}`}
